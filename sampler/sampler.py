...
class AbstractSampler(object):
    ...
    def _uni_sampling(self, sample_num, field):
    ...
    def sampling(self, sample_num, field=None):
        ...
        if self.distribution == 'uniform':
            return self._uni_sampling(sample_num, field=field)
    ...
    def sample_by_key_ids(self, key_ids, num, field=None):
        """Sampling by key_ids.
        ...
        if (key_ids == key_ids[0]).all():
            ...
            value_ids = self.sampling(total_num, field=field)
            ...
            while len(check_list) > 0:
                value_ids[check_list] = self.sampling(len(check_list), field=field)
                check_list = np.array([
...
class RepeatableSampler(AbstractSampler):
    ...
    def __init__(self, phases, dataset, distribution='uniform'):
        ...
        self.tag_num = dataset.tag_num

        super().__init__(distribution=distribution)

    def _uni_sampling(self, sample_num, field='item'):
        if field == 'tag':
            return np.random.randint(1, self.tag_num, sample_num)
        else:
            return np.random.randint(1, self.item_num, sample_num)
    ...
    def _get_tag_candidates_list(self):
        return list(self.dataset.inter_feat[self.tid_field].numpy())
    ...
    def sample_by_user_ids(self, user_ids, item_ids, num, field=None):
        """Sampling by user_ids.
        ...
        try:
            ...
            return self.sample_by_key_ids(np.arange(len(user_ids)), num, field=field)
        except IndexError:
            for user_id in user_ids:
                if user_id < 0 or user_id >= self.user_num:
                    raise ValueError(f'user_id [{user_id}] not exist.')
