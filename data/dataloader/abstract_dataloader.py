# -*- coding: utf-8 -*-
# @Filename: abstract_dataloader.py
...
class NegTagSampleDataLoader(AbstractDataLoader):
    ...
    def _set_neg_sample_args(self, config, dataset, dl_format, neg_sample_args):
        ...
        self.tid_field = dataset.tid_field
        ...
        if self.neg_sample_args['strategy'] == 'by':
            ...
            if self.dl_format == InputType.POINTWISE:
                ...
                self.neg_tag_id = self.neg_prefix + self.tid_field

                columns = [self.iid_field] if dataset.item_feat is None else dataset.item_feat.columns
                tag_columns = [self.tid_field] if dataset.tag_feat is None else dataset.tag_feat.columns
                ...
                for tag_feat_col in tag_columns:
                    neg_tag_feat_col = self.neg_prefix + tag_feat_col
                    dataset.copy_field_property(neg_tag_feat_col, tag_feat_col)
    ...
    def _neg_sampling(self, inter_feat):
        if 'dynamic' in self.neg_sample_args.keys() and self.neg_sample_args['dynamic'] != 'none':
            ...
            user_ids = inter_feat[self.uid_field].numpy()
            item_ids = inter_feat[self.iid_field].numpy()
            tag_ids = inter_feat[self.tid_field].numpy()
            neg_item_ids = self.sampler.sample_by_user_ids(user_ids, item_ids, self.neg_sample_num, field='item')
            neg_tag_ids = self.sampler.sample_by_user_ids(user_ids, tag_ids, self.neg_sample_num, field='tag')
            return self.sampling_func(inter_feat, neg_item_ids, neg_tag_ids)
        else:
            return inter_feat

    def _neg_sample_by_pair_wise_sampling(self, inter_feat, neg_item_ids, neg_tag_ids):
        ...
        neg_tag_feat = Interaction({self.tid_field: neg_tag_ids})
        neg_tag_feat = self.dataset.join(neg_tag_feat)
        neg_tag_feat.add_prefix(self.neg_prefix)
        inter_feat.update(neg_tag_feat)
        return inter_feat

    def _neg_sample_by_point_wise_sampling(self, inter_feat, neg_item_ids, neg_tag_ids):
