...
def save_split_tag_dataloaders(config, dataloaders):
...
def tag_data_preparation(config, dataset):
    ...
    if dataloaders is not None:
        ...
        if config['save_dataloaders']:
            save_split_tag_dataloaders(config, dataloaders=(train_data, valid_data, test_data))
    ...
    if phase == 'train':
        if model_type != ModelType.KNOWLEDGE:
            if config['tag_neg_sampling']:
                return general_tag_dataloader.TrainDataLoader
            else:
